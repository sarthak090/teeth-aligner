...
const designPricingList = [
  { list: 'Virtual consultation with orthodontist', feature: true },
  { list: '3D scan of your teeth', feature: true },
  { list: 'Custom aligner treatment plan', feature: true },
  { list: 'Set of clear aligners', feature: true },
  { list: 'Progress tracking app', feature: true },
  { list: 'Virtual check-ins', feature: true },
  { list: 'In-person appointments', feature: false },
  { list: 'Emergency support', feature: true },
]

const customPricingList = [
  { list: 'In-clinic consultation with orthodontist', feature: true },
  { list: '3D scan of your teeth', feature: true },
  { list: 'Custom aligner treatment plan', feature: true },
  { list: 'Set of clear aligners', feature: true },
  { list: 'Progress tracking app', feature: true },
  { list: 'Regular in-person check-ups', feature: true },
  { list: 'Emergency support', feature: true },
  { list: 'Treatment refinements', feature: true },
]
...
interface PricingProps {
  ...
  packages: {
    package_1:{
      ...
      description:string;
      price:string;
      features:{feature:string,is_feature:'Yes'|'No'}[];
    }
    package_2:{
      name:string;
      description:string;
...
const PricingCard = ({ showHeader = false,packages }: PricingProps) => {
  ...
  const package_1 = packages.package_1
  const package_2 = packages.package_2
      ...
      <div className="container" id='pricing'>
        <div>
          ...
          <div className="mt-10 grid justify-center gap-x-10 gap-y-10 md:mt-20 lg:grid-cols-2">
            <RevealWrapperV2 className="border border-primary px-[30px] pb-[30px] pt-8 dark:border-backgroundBody md:pt-20">
              <img src="/images/pricing-gradient-bg-1.png" className="absolute left-0 top-0" alt="Gradient color" />
              <div>
                <div>
                  <h6 className="mb-4 max-md:text-3xl">{package_1.name}</h6>
                ...
                <div className="flex flex-col items-start gap-x-10 gap-y-3 py-8 md:flex-row md:items-end md:gap-y-6">
                  <p className="text-6xl leading-[60px] text-secondary dark:text-backgroundBody">${package_1.price}</p>
                ...
                <ul className="mt-8 md:ml-7 [&>*:not(:last-child)]:mb-2 md:[&>*:not(:last-child)]:mb-3">
                  {package_1.features.map((priceList, idx) => (
                    <li
                      key={`list-${idx}`}
            ...
            <RevealWrapper className="border px-[30px] pb-[30px] pt-8 dark:border-dark md:pt-20">
              ...
              <div>
                <div>
                  <h6 className="mb-4 max-md:text-3xl">{package_2.name}</h6>
                  <p className="text-base">
                ...
                {pricingToggle ? (
                  <div>
                    <div className="flex flex-col items-start gap-x-10 gap-y-3 py-8 md:flex-row md:items-end md:gap-y-6">
                      <p className="text-6xl leading-[60px] text-secondary dark:text-backgroundBody">${package_2.price}</p>
                      <p className="text-sm leading-[18px] tracking-[-0.2px]">
                        One-time payment <br />
                        Flexible payment plans available
                      </p>
                ...
                <ul className="mt-8 md:ml-7 [&>*:not(:last-child)]:mb-2 md:[&>*:not(:last-child)]:mb-3">
                  {package_2.features.map((priceList, idx) => (
                    <li
                      key={`list-${idx}`}
                      className="flex list-none gap-[10px] text-[17px] leading-[1.5] text-secondary/70 dark:text-backgroundBody/70">
                      {true ? (
                        ...
                        <span>
                          <svg
                            ...
                            fill="none">
                            <path
                              d="M16.2806 10.0045C16.3504 10.0742 16.4057 10.1569 16.4434 10.248C16.4812 10.339 16.5006 10.4366 16.5006 10.5352C16.5006 10.6337 16.4812 10.7313 16.4434 10.8224C16.4057 10.9134 16.3504 10.9961 16.2806 11.0658L11.0306 16.3158C10.961 16.3855 10.8783 16.4408 10.7872 16.4786C10.6962 16.5163 10.5986 16.5357 10.5 16.5357C10.4014 16.5357 10.3038 16.5163 10.2128 16.4786C10.1218 16.4408 10.039 16.3855 9.96938 16.3158L7.71938 14.0658C7.57865 13.9251 7.49959 13.7342 7.49959 13.5352C7.49959 13.3361 7.57865 13.1453 7.71938 13.0045C7.86011 12.8638 8.05098 12.7847 8.25 12.7847C8.44903 12.7847 8.6399 12.8638 8.78063 13.0045L10.5 14.7248L15.2194 9.21937C15.289 9.14969 15.3718 9.09442 15.4628 9.0567C15.5539 9.01899 15.6515 8.99958 15.75 8.99958C15.8486 8.99958 15.9461 9.01899 16.0372 9.0567C16.1282 9.09442 16.2109 9.14969 16.2806 9.21937C16.3503 9.28906 16.4056 9.37178 16.4433 9.46283C16.481 9.55387 16.5004 9.65145 16.5004 9.75C16.5004 9.84855 16.481 9.94613 16.4433 10.0372C16.4056 10.1282 16.3503 10.2109 16.2806 10.2806ZM21.75 12.7852C21.75 14.7135 21.1782 16.5986 20.1068 18.202C19.0355 19.8053 17.5127 21.055 15.7312 21.793C13.9496 22.5309 11.9892 22.724 10.0979 22.3478C8.20656 21.9716 6.46928 21.043 5.10571 19.6794C3.74215 18.3159 2.81355 16.5786 2.43735 14.6873C2.06114 12.796 2.25422 10.8356 2.99218 9.05399C3.73013 7.27241 4.97982 5.74967 6.58319 4.67833C8.18657 3.60698 10.0716 3.03516 12 3.03516C14.585 3.03789 17.0634 4.06599 18.8913 5.89388C20.7192 7.72177 21.7473 10.2001 21.75 12.7852ZM20.25 12.7852C20.25 11.1535 19.7661 9.55841 18.8596 8.2017C17.9531 6.845 16.6646 5.78757 15.1571 5.16315C13.6497 4.53873 11.9909 4.37535 10.3905 4.69368C8.79017 5.01201 7.32016 5.79774 6.16637 6.95153C5.01259 8.10531 4.22685 9.57532 3.90853 11.1757C3.5902 12.776 3.75358 14.4348 4.378 15.9423C5.00242 17.4498 6.05984 18.7383 7.41655 19.6448C8.77326 20.5513 10.3683 21.0352 12 21.0352C14.1873 21.0327 16.2843 20.1627 17.8309 18.616C19.3775 17.0694 20.2475 14.9724 20.25 12.7852Z"
                              fill="black"
                            />
                          </svg>
                          <svg
