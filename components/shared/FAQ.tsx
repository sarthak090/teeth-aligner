...
const FAQ = ({ bigTitleWithBadge = false, data }: FaqProps) => {
      ...
      <div className="container">
        ...
        <RevealWrapper className="mx-auto w-full max-w-[900px] [&>*:not(:last-child)]:mb-6">
          {data.map((item, index) => (
            <div
              key={index}
              className={`faq-body-transition overflow-hidden border border-primary  bg-backgroundBody duration-[400ms] dark:bg-dark ${
                activeIndex === index ? 'pb-10' : 'pb-0'
              }`}
              style={{
                borderColor: activeIndex === index ? '#ff8200' : 'transparent ',
              }}>
              <div
                ...
                <div
                  className={`accordion-header-icon  transition-transform duration-[400ms] ${
                    activeIndex === index ? 'rotate-180 !bg-primary !border-primary' : 'rotate-0 '
                  }`}></div>
              </div>
              <div
 ...
 {
          bigTitleWithBadge && <RevealWrapper as="ul" className="mx-auto mt-[56px] flex list-none justify-center">
